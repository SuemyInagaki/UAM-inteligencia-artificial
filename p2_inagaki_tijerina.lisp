...
(defun navigate (city edges)
  (mapcar #'(lambda (lista) (make-action :origin (first lista) 
                                         :final (second lista) 
                                         :cost (third lista))) 
...
;es una funcion que crea una lista de ciudades obligatorias por donde
;un nodo ha pasado
...
; esa funcion verifica si dos listas de caminos son iguales
; path1 y path2 tienen que estar ordenados
; path1 y path2 son listas de ciudades obligatorias por donde node1
; y node2 han pasado, respectivamente
; si path1 y path2 tuvieren tamaños diferentes, entonces la funcion retorna 
; NIL

(defun verifica-condicion-busqueda (path1 path2)
  (if (and (null path1) (null path2))
      T
...
; tem que verificar se mandatory es null
      ...
      (if (null mandatory)
          ...
          (and (equal (node-city node-1) (node-city node-2))
               (verifica-condicion-busqueda 
                  (sort (crea-lista-ciudades mandatory (crea-camino node-1)) #'string-lessp)
                  (sort (crea-lista-ciudades mandatory (crea-camino node-2)) #'string-lessp))))))
...
(defun succ (node) (navigate (node-city node) *trains*))

(defun goal-test (node) (f-goal-test node *destination* *mandatory*))

(defun call-sse (node1 node2) (f-search-state-equal node1 node2 *mandatory*))

(defun call-f-h (city) (f-h city *heuristic*))
...
(defun insert-node (node lst1 node-compare-p)
  (if (null lst1)
    ...
    (if (funcall node-compare-p node (car lst1)) 
      (cons node lst1)
      (cons (car lst1) (insert-node node (cdr lst1) node-compare-p)))))
      ...
      (if (exp-cond (car open-nodes) closed-nodes)
        (graph-search-aux 
          (node-in-lst 
            ...
            (cdr open-nodes)
            strategy)
            (insert-node (car open-nodes) closed-nodes (strategy-node-compare-p strategy))
...
;; eu acredito que eu tenho que criar uma função
;; junto com o exercicio 9, onde vá guardando os
;; caminhos. Preciso da solução para poder fazer o solution path
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; 
;;;    BEGIN Exercise 11: Depth and Breadth search
...
(defun breadth-first-node-compare-p (node-1 node-2)
  (if (or (null (node-parent node-1)) (null (node-parent node-2)))
    ...
    (<= (node-depth (node-parent node-1)) (node-depth (node-parent node-2)))))
...
;;; 
;;;    END Exercise 11: depth and breadth first node compare p
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
